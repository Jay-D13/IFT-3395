...
class SVM:
    ...
    def compute_loss(self, x, y):
        ...
        returns : float
        """
        loss = np.sum(np.maximum(0,2-y*np.dot(x,self.w))**2,axis=1).mean() + 0.5*self.C * np.sum(self.w**2)
        return loss
    ...
    def compute_gradient(self, x, y):
        ...
        """
        activation = np.maximum(0,2-y*np.dot(x,self.w))
        return 2/x.shape[0] * np.dot(-x.T, y*activation) + self.C * self.w
    ...
    def infer(self, x):
        ...
        """
        return self.make_one_versus_all_labels(np.argmax(np.dot(x,self.w), axis=1), self.m)
    ...
    def compute_accuracy(self, y_inferred, y):
        ...
        returns : float
        """
        differing_labels = np.count_nonzero(y - y_inferred, axis=1)
        score = differing_labels == 0
        return sum(score)/y.shape[0]
...
if __name__ == "__main__":
    ...
    plt.plot(train_accs,train_losses)
